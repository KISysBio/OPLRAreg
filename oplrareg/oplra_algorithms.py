...
class OplraRegularised(BaseOplraEstimator):
    ...
    def __init__(self, lam=0.005, epsilon=0.01, beta=0.03, exact_number_regions=None, solver_name="glpk"):
    ...
    def predict(self, X):
        ...
        if self.final_model.number_regions > 1:
            ts_region = self.final_model.get_regions(X.ix[:, self.final_model.fStar])
        else:
        ...
        if len(counter_region) == 1:
            final_predictions = safe_sparse_dot(X, coefficients[0], dense_output=True) + intercepts[0]
        else:
            for region, counter_region in counter_region.items():
                samples_in_region = np.argwhere(ts_region == region).transpose()[0]
                final_predictions[samples_in_region] = safe_sparse_dot(X.iloc[samples_in_region], coefficients[region],
                                                                       dense_output=True) + intercepts[region]
    ...
    def fit(self, X, y, f_star=None, verbose=True):
        ...
        if self.beta is None or self.exact_number_regions == 1:
            ...
            tmp_model, termination_condition = self.run_model(X, y, number_regions, f_star, selected_features)
        ...
        if f_star is None:
            ...
            if verbose:
                print("\n%s\n########## R = %d f* = %s ###########" % (self, number_regions, f_star))
    ...
    def get_model_info(self):
        ...
        for region in range(self.final_model.number_regions):
            ...
            coefficients = coefficients.append(pd.DataFrame(data=values, index=np.arange(1), columns=cols),
                                               ignore_index=True)

        if self.final_model.number_regions == 1:
            breakpointsDF = pd.DataFrame(data={'fStar': None, 'breakpoints': None}, index=np.arange(1))
        else:
            breakpointsDF = pd.DataFrame(
                {'fStar': self.final_model.fStar, 'breakpoints': self.final_model.get_breakpoints()})
