...
from pyomo.core.base import ConcreteModel, Var, Set, NonNegativeReals, \
    Binary, Constraint, Objective, minimize, Reals
...
class OplraPyomoModel(ConcreteModel):
    ...
    def __init__(self, data, target, number_regions, lam, epsilon=0.01, f_star=None, selected_features=None):
        super().__init__(name="OplraRegression %d regions (f*=%s)" % (number_regions, f_star))
        ...
        if self.number_regions == 1:
            ...
            self.breakpoint_order  = Constraint(self.r, rule=OplraPyomoModel.breakpoint_order_rule)
            self.region_divider1   = Constraint(self.r, self.s,rule=OplraPyomoModel.region_divider1_rule)
            self.region_divider2   = Constraint(self.r, self.s,rule=OplraPyomoModel.region_divider2_rule)

        #Common constraints
        self.sample_to_region  = Constraint(self.s, rule=OplraPyomoModel.sample_to_region_rule)
        self.prediction        = Constraint(self.r, self.s,rule=OplraPyomoModel.prediction_rule)
        self.abs_error1        = Constraint(self.r, self.s,rule=OplraPyomoModel.abs_error1_rule)
        self.abs_error2        = Constraint(self.r, self.s,rule=OplraPyomoModel.abs_error2_rule)
        self.abs_coeff1        = Constraint(self.r, self.f, rule=OplraPyomoModel.abs_coeff1_rule)
        self.abs_coeff2        = Constraint(self.r, self.f, rule=OplraPyomoModel.abs_coeff2_rule)
    ...
    def get_regions(self, f_star_series):
        ...
        ts_distance = [[max(0, f_star_series[s] - up_r[r], lo_r[r] - f_star_series[s])
                       for r in regions] for s in samples]
    ...
    def get_coefficients(self):
        ...
        return np.vstack([self.W[(r, f)].value if f in self.f else 0 for f in self.data.columns] for r in self.r)
    ...
    def region_divider1_rule(model, r, s):
        if (r == model.number_regions - 1):
            ...
            return (model.X[r] - model.epsilon +
                    model.U1 * (1 - model.F[r, s])) >= model.data.loc[s, model.fStar]

    def region_divider2_rule(model, r, s):
        if (r == 0):
            return Constraint.Skip
        else:
            return (model.X[r-1] + model.epsilon -
                    model.U1 * (1 - model.F[r, s])) <= model.data.loc[s,model.fStar]
    ...
    def prediction_rule(model, r, s):
        return model.Pred[r, s] == (sum(model.data.loc[s, ff] * model.W[r, ff] for ff in model.f) + model.B[r])

    def abs_error1_rule(model, r, s):
        return model.D[s] >= (model.target[s] - model.Pred[r, s] - model.U2 * (1 - model.F[r, s]))

    def abs_error2_rule(model, r, s):
        return model.D[s] >= (model.Pred[r, s] - model.target[s] - model.U2 * (1 - model.F[r, s]))
    ...
    def regularisation_rule(model):
        return model.reg == model.lam * sum(model.absW[r, f] for r in model.r for f in model.f)
